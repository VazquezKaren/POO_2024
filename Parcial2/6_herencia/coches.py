...
class Camiones(Coches):
   ...
   def cargar(self, tipo_caraga):
      self.__tipo_carga=tipo_caraga
...
class Camionetas (Coches):
   ...
   def carga(self,cantidadCarga):
      self.cantidadCarga=cantidadCarga
      return self.cantidadCarga
   ...
   def getInfo(self):
        print(f"Marca: {self.getMarca()} {self.getColor()}, numeros de plazas: {self.getPlazas()} \nModelo: {self.getModelo()} con una velocidad de {self.getVelocidad()} Km/h y un potencia de {self.getCaballaje()} hp, con traccion {self.getTraccion()} y cerrada {self.getCerrada()} ") 
